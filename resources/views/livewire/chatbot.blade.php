<div class="fixed bottom-20 right-4 z-50 md:bottom-6 md:right-6">
    ...
    <button wire:click="toggleChat"
        class="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white border-2 border-[#3A9E3F] text-[#3A9E3F] shadow-xl flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 z-50 relative">
        @if($isOpen)
        {{-- Ikon 'X' (Tutup) biarkan pakai SVG bawaan agar rapi --}}
        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
        @else
        {{-- Ganti SVG Chat dengan gambar PNG AI Anda --}}
        <img src="{{ asset('images/iconai.png') }}"
            alt="AI Assistant"
            class="w-7 h-7 object-contain animate-pulse" />
        {{-- Hapus style filter di atas jika file PNG Anda sudah berwarna putih transparan --}}
        @endif
    </button>
